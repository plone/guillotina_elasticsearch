...
from guillotina_elasticsearch.interfaces import (
    IElasticSearchUtility,
)  # noqa b/w compat import
...
class ElasticSearchUtility(DefaultSearchUtility):
    ...
    async def reindex_all_content(
        ...
        reindexer = Reindexer(
            self, obj, response=response, reindex_security=security, request=request
        )
    ...
    async def search(
        ...
        final = {"items_count": result["hits"]["total"]["value"], "member": items}
